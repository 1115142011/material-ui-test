  ...
  return (
    <GroupedVirtuoso
      ...
      GroupContainer={({ children, className, style, ...props }) => (
        <ListSubheader {...props} className={[className, classes.header].join(' ')} disableSticky={true} style={{ ...style, marginBottom: 0 }}>
          {children}
        </ListSubheader>
      )}
      ...
      item={(index) => (
        <>
          ...
          <ListItemText primary={`${users[index].name}`} secondary={<span>{users[index].description}</span>} className={classes.text} />
        </>
      )}
      footer={() => {
